...
class Test(SMTestCase):
    ...
    def _check_prediction_derivatives(self,sm):
        ...
        pred_errors= np.array( [np.abs((diff_g-deriv[0][0])/diff_g),np.abs((diff_d-deriv[0][1])/diff_d)])
        ...
        deriv = np.array([sm.predict_derivatives(x, 0)[0], sm.predict_derivatives(x, 1)[0]]).T
        pred_errors= np.array( [np.abs((diff_g-deriv[0][0])/diff_g),np.abs((diff_d-deriv[0][1])/diff_d)])
        total_error =  np.sum(pred_errors**2)
