...
class Test(SMTestCase):
    def setUp(self):
        ...
        self.X = X
        self.y = y
        self.X_norma, self.y_norma,  self.X_mean,  self.y_mean,  self.X_std,  self.y_std = X_norma, y_norma, X_mean, y_mean, X_std, y_std
        self.D, self.ij =  D, ij 
        self.theta = theta
    ...
    def test_corr_derivatives(self):
        for ind, corr in enumerate(self.corr_def) :#For every kernel
            ...
            self.assert_error(np.array(grad_norm_all), np.array(diff_norm_all), 1e-5, 1e-5) #from utils/smt_test_case.py
    ...
    def test_likelihood_derivatives(self):
        for corr_str in ['abs_exp','squar_exp','act_exp','matern32','matern52']:#For every kernel 
          for poly_str in ['constant', 'linear','quadratic']: #For every method
              ...
              self.assert_error(grad_norm_all, diff_norm_all, atol=1e-5, rtol=1e-3) #from utils/smt_test_case.py
            ...
            for poly_str in ['constant', 'linear', 'quadratic']:#For every method
                 ...
                 for j, omega_j in enumerate(theta):
                     ...
                     for i, theta_i in enumerate(theta):
                         ...
                         grad_norm_all.append(np.linalg.norm(Hess[i,j]) / np.linalg.norm(Hess))
                         diff_norm_all.append(np.linalg.norm(hess_eps) / np.linalg.norm(Hess))
                         ind_theta.append(r'$x_%d,x_%d$'%(j,i))     
                 self.assert_error(np.array(grad_norm_all), np.array(diff_norm_all), atol=1e-5, rtol=1e-3) #from utils/smt_test_case.py
