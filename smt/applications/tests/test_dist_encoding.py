...
class TestDistEncoding(unittest.TestCase):
    def test_dist_encoding_krg(self):
        ...
        y = np.concatenate([
            rng.normal(1.0, 0.2, n_per_level),
            rng.normal(5.0, 0.2, n_per_level),
            rng.normal(10.0, 0.2, n_per_level)
        ]).reshape(-1, 1)
        ...
        self.assertGreater(corr, 0.9, "Predictions should be highly correlated with levels")
    ...
    def test_dist_encoding_beta_effect(self):
        ...
        y = np.concatenate([
            rng.normal(0.0, 0.1, n_per_level),
            rng.normal(5.0, 0.1, n_per_level)
        ]).reshape(-1, 1)
        
        ds = DesignSpace([CategoricalVariable(values=["A", "B"])])
        
        sm1 = KRG(design_space=ds, categorical_kernel=MixIntKernelType.DIST_ENCODING, 
                  categorical_kernel_beta=1.0, print_global=False, n_start=5, seed=42)
        sm1.set_training_values(X, y)
        sm1.train()
        
        sm2 = KRG(design_space=ds, categorical_kernel=MixIntKernelType.DIST_ENCODING, 
                  categorical_kernel_beta=2.0, print_global=False, n_start=5, seed=42)
    ...
    def test_grouped_levels_da_veiga_reproduction(self):
        ...
        def complex_fun(x, r):
            ...
            y[(x_cat >= 4) & (x_cat < 8)] = 1.2 * x_cont[(x_cat >= 4) & (x_cat < 8)] # Group 2
        ...
        ytrain = complex_fun(xtrain, rng)

        # Fit DIST_ENCODING (DE)
        sm_de = KRG(design_space=ds, categorical_kernel=MixIntKernelType.DIST_ENCODING, 
                    print_global=False, n_start=5, seed=42)
        sm_de.set_training_values(xtrain, ytrain)
        sm_de.train()

        # Fit GOWER (Standard)
        sm_gower = KRG(design_space=ds, categorical_kernel=MixIntKernelType.GOWER, 
                       print_global=False, n_start=5, seed=42)
        ...
        # Evaluate
        x_cont_test = lhs(50)
        x_cat_test = rng.choice(np.arange(n_levels), 50).reshape(-1, 1)
        xtest = np.hstack([x_cat_test, x_cont_test])
        ytest = complex_fun(xtest, rng)

        mse_de = np.mean((ytest - sm_de.predict_values(xtest)) ** 2)
        mse_gower = np.mean((ytest - sm_gower.predict_values(xtest)) ** 2)

        # DE should outperform Gower on structured mixed problems because Gower
        # cannot see that some levels (Group 1 and 2) are more similar than others.
        self.assertLess(mse_de, mse_gower, "DE should outperform Gower on structured mixed problems")
    ...
    def test_learning_curve(self):
        ...
        def fun(x, r):
            ...
            return offsets[x_cat].reshape(-1, 1) + r.normal(0, 0.01, (len(x), 1))

        mses = []
        xtest = np.array([[0], [1], [2]])
        # Clean test set for curve evaluation
        ytest = np.array([[0.0], [5.0], [10.0]])

        for n_per_cat in [2, 5, 10]:
            xtrain = np.repeat([0, 1, 2], n_per_cat).reshape(-1, 1)
            ytrain = fun(xtrain, rng)

            sm = KRG(design_space=ds, categorical_kernel=MixIntKernelType.DIST_ENCODING, 
                     categorical_kernel_beta=1.0, print_global=False, n_start=1, seed=42)
