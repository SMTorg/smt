...
class TestMFKmixed(unittest.TestCase):
    ...
    def g_DTLZ5(self,x):
        ...
        out = (x1-0.5)**2 + (x2-0.5)**2 + (x3-0.5)**2 + (x4-0.5)**2 + (x5-0.5)**2
        return out
    ...
    def h_ZDT3(self,x):
        ...
        out = 1 - np.sqrt(x0/self.g_ZDT(x)) - (x0/self.g_ZDT(x))*np.sin(10*np.pi*x0)
        return out
    ...
    def run_mfk_mixed_example(self):
        ...
        ds = DesignSpace([
            ...
            CategoricalVariable(['blue', 'red', 'green']),  # x5 categorical: blue, red or green; order is not relevant
            ])    
        ...
        for krg_method in KRG_METHODS:
            ...
            if("mfk" in krg_method): # if multifi compute LF validation data
                ...
                xt_LF = np.concatenate((xt_HF, sampling.expand_lhs(xt_HF,  n_train_LF-n_train_HF)), axis=0)
            ...
            if("mfk" in krg_method):
                print("norme2(y1_predict_LF - y1_valid_LF) =", npl.norm(y1_predict_LF - y1_valid_LF,2))
            ...
            if("mfk" in krg_method):
                print("norme2(y2_predict_LF - y2_valid_LF) =", npl.norm(y2_predict_LF - y2_valid_LF,2))
        ...
        #------------------------------------------------------------------------------
            ...
            if("mfk" in krg_method):
                ...
                plt.scatter(y1t_LF, np.ravel(sm1._predict_intermediate_values(xt_LF,1)))
                plt.plot(np.linspace(0, 1.5, 100), np.linspace(0, 1.5, 100), color="red")
                plt.show()
                ...
                plt.scatter(y2t_LF, np.ravel(sm2._predict_intermediate_values(xt_LF,1)))
                plt.plot(np.linspace(0, 9, 100), np.linspace(0, 9, 100), color="red")
                plt.show()
                #------------------------------------------------------------------------------
