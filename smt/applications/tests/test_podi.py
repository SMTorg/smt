...
class Test(SMTestCase):
    ...
    def gram_schmidt(polynomials): #gram_schmidt
        ...
        for i in range(len(polynomials)):
            ...
            for j in range(i):
                basis[i] -= np.dot(polynomials[i], basis[j]) / np.dot(basis[j], basis[j]) * basis[j]
    ...
    def test_set_options(self):
        ...
        options = [{'poly': 'quadratic', 'corr': 'matern32', 'pow_exp_power': 0.38, 'theta0': [1e-1]}]
    ...
    def test_pod(self):
        ...
        np.testing.assert_allclose(singular_values[n_mods:], np.zeros(self.nt - n_mods), atol = 1e-9)
