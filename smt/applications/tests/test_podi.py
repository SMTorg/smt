...
class Test(SMTestCase):
    ...
    def run_podi_example_2d_local():
        ...
        def choose_local_bases(local_pod_bases, n_bases, modes_list, xt1, xv1):
            ...
            for value in xv1:
                sorted_ind = sorted(range(xt1.shape[0]), key = lambda k: abs(xt1[:,0] - value)[k])
                ...
                basis = PODI.interp_subspaces(  xt1 = np.atleast_2d(keep_xt1).T,
                                                input_matrices = input_matrices,
        ...
        podi.compute_pod(database = dbt[:, j], pod_type = "local", local_basis = interpolated_bases[i])
