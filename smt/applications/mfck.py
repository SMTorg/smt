        ...
        if self.lvl == 1:
            ...
            for lvel in range(self.lvl):
                if lvel == 0:
                    ...
                    lower_bounds = np.hstack((self.options["sigma_bounds"][0], np.full(self.nx, self.options["theta_bounds"][0])))
                    upper_bounds = np.hstack((self.options["sigma_bounds"][1], np.full(self.nx, self.options["theta_bounds"][1])))
                    ...
                    lower_boundst = np.hstack((self.options["sigma_bounds"][0], np.full(self.nx, self.options["theta_bounds"][0])))
                    upper_boundst = np.hstack((self.options["sigma_bounds"][1], np.full(self.nx, self.options["theta_bounds"][1])))
            ...
            if lvel == 0:
                ...
                if lvel == 1:
                    ...
                    for i in range (lvel,self.lvl-1):
                        a = str(lvel)
                        b=str(i+1)
                        Kb[a+b]=np.prod(rhos[lvel:i+1])*(param[-1]**2 * get_K(lvel,i+1, param0[:2]) + get_K(lvel,i+1, param[:-1]))
                else:
                    ...
                    for i in range (lvel,self.lvl-1):
                        a = str(lvel)
                        b=str(i+1)
                        temp=param_ant[-1] ** 2 * get_K(lvel,i+1, param0[:2]) + (get_K(lvel,i+1, param_ant[:-1]) if k2 is not None else 0)
