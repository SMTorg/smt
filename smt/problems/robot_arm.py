...
class RobotArm(Problem):
    ...
    def _initialize(self):
        ...
        # Length L
        self.xlimits[0::2, 0] = 0.
        self.xlimits[0::2, 1] = 1.

        # Angle theta
        self.xlimits[1::2, 0] = 0.
        ...
        if kx is None:
            ...
            if kx % 2 == 0:
                ...
                for iseg in range(nseg):
                    L = x[:, 2*iseg + 0]
                    if kseg <= iseg:
                        d_pos_x[:] -= L * np.sin(np.sum(x[:, 1:2*iseg+2:2], axis=1))
                        d_pos_y[:] += L * np.cos(np.sum(x[:, 1:2*iseg+2:2], axis=1))
                y[:, 0] += pos_x / (pos_x ** 2 + pos_y ** 2) ** 0.5 * d_pos_x
                y[:, 0] += pos_y / (pos_x ** 2 + pos_y ** 2) ** 0.5 * d_pos_y
