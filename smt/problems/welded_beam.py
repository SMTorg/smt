...
Author: Dr. Mohamed Amine Bouhlel <mbouhlel@umich.edu>
       Dr. John T. Hwang         <hwangjt@umich.edu>Welded beam problem from:
...
import numpy as np
from scipy.misc import derivativefrom smt.problems.problem import Problemclass WeldedBeam(Problem):    def _declare_options(self):
       self.options.declare('name', 'WeldedBeam', types=str)
       self.options.declare('use_FD', False, types=bool)
       self.options['ndim'] = 3    def _initialize(self):
       assert self.options['ndim'] == 3, 'ndim must be 3'        # t, h, l
       self.xlimits[:, 0] = [5,0.125,5]
       self.xlimits[:, 1] = [10,1,10]    def _evaluate(self, x, kx):
       """
       Arguments
       ---------
       x : ndarray[ne, nx]
       ...
       kx : int or None
           ...
           None means return function value rather than derivative.        Returns
       -------
       ndarray[ne, 1]
           Functions values if kx=None or derivative values if kx is an int.
       """
       ne, nx = x.shape        y = np.zeros((ne, 1), complex)        def partial_derivative(function, var=0, point=[]):
           args = point[:]
           def wraps(x):
               args[var] = x
               return func(*args)
           return derivative(wraps, point[var], dx = 1e-6)        def func(x0,x1,x2):
           tau1 = 6000/(np.sqrt(2)*x1*x2)
           tau2 = 6000*(14+0.5*x2)*np.sqrt(0.25*(x2**2+(x1+x0)**2)) \
               / (2*(0.707*x1*x2*(x2/12.+0.25*(x1+x0)**2)))
           return np.sqrt(tau1**2+tau2**2+x2*tau1*tau2/np.sqrt(0.25*(x2**2+(x1+x0)**2)))        for i in range(ne):
           x0 = x[i,0]
           x1 = x[i,1]
           x2 = x[i,2]
           if kx is None:
               ...
               if self.options['use_FD']:
                   ...
                   point[kx] -= complex(0, ch)        return y
