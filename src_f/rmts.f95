...
subroutine compute_full_hess(nnz, nnz_map, nterm, nelem, hblock, &
    map_data, map_row_meta, map_cols, data, rows, cols)

  implicit none

  !f2py intent(in) nnz, nnz_map, nterm, nelem, hblock, map_data, map_row_meta, map_cols
  !f2py intent(out) data, rows, cols
  !f2py depend(nterm) hblock
  !f2py depend(nnz_map) map_data, map_cols
  !f2py depend(nterm, nelem) map_row_meta
  !f2py depend(nnz) data, rows, cols

  ! Input
  integer, intent(in) :: nnz, nnz_map, nterm, nelem
  double precision, intent(in) :: hblock(nterm, nterm), map_data(nnz_map)
  integer, intent(in) :: map_row_meta(nterm * nelem + 1), map_cols(nnz_map)
  ...
  integer :: nnz_block, inz_block1, inz_block2, inz_offset, irow_offset

  inz = 0
  do ielem = 1, nelem
    irow_offset = (ielem - 1) * nterm
    inz_offset = map_row_meta(irow_offset + 1)
    nnz_block = map_row_meta(irow_offset + nterm + 1) - inz_offset

    irow1 = irow_offset + 1
    do inz_block1 = 1, nnz_block
      do while (map_row_meta(irow1 + 1) .le. inz_offset + inz_block1 - 1)
        irow1 = irow1 + 1
      end do
      icol1 = map_cols(inz_block1) + 1

      irow2 = irow_offset + 1
      do inz_block2 = 1, nnz_block
        do while (map_row_meta(irow2 + 1) .le. inz_offset + inz_block2 - 1)
          irow2 = irow2 + 1
        end do
        icol2 = map_cols(inz_block2) + 1

        prod = hblock(irow1 - irow_offset, irow2 - irow_offset)
        prod = prod * map_data(inz_offset + inz_block1)
        prod = prod * map_data(inz_offset + inz_block2)
  ...
  if (inz .ne. nnz) then
     print *, 'Error: incorrect nnz in compute_full_hess', inz, nnz
  end if

end subroutine compute_full_hess
