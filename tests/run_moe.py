...
from smt.mixture.moe import MOE
...
#         plt.show()
        ...
        # sample construction
        mixture = MOE()
        mixture.options['number_cluster'] = 4 
        mixture.options['X'] = x_train
        mixture.options['y'] = np.array(y_train)

        # mixture of expert
#         mixture.fit(dim, x_train, y_train, y_train, detail=False,
#                     plot=False, heaviside=True, number_cluster=0, median=True)

        mixture.apply_method()
        ...
        mixture = MOE(hard_recombination=False)
        mixture.options['X'] = x_train
        mixture.options['y'] = np.array(y_train)
