...
hyperparameters defined as a density probabilty function. Especially we suppose that the function
...
matrix :math:`A` is an hyperparameter. Thus we have :math:`d_e \cdot d` hyperparameters to find.
Moreover, we suppose that :math:`A` follow a normal distribution :math:`\mathcal{N}(vect(\hat A),\Sigma)`,
with a mean :math:`\hat A` and a covariance matrix :math:`\Sigma`, which is taken into account in the Gaussian Process building.
Using a kriging model using :math:`\hat A` as hyperparameters and some approximations, 
...
::
  ...
  ___________________________________________________________________________
   ...
   Training
     ...
     Training ...
     Training - done. Time (sec):  0.1087687
