...
With :math:`k \in [\![1,N]\!]`, :math:`u(\mathbf{x_k})` corresponds to the output values of the output at a specific configuration. It is called a **snapshot**.
...
Each column of the matrix corresponds to a snapshot :math:`u(\mathbf{x_k})`.
